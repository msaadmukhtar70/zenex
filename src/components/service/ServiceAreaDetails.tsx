...
interface Props {
  data: string[]; // Accept an array of feature strings
}

const ServiceAreaDetails = ({ data }: Props) => {
  return (
    <>
          ...
          <div className="cs_work cs_work_1">
            <div className="cs_card_work cs_style_1">
              {data.map((feature, index) => (
                <div key={index} className="cs_card cs_mt_nthchild_0 anim_div_ShowLeftSide">
                  ...
                  <h6 className="cs_work_title">Step {index + 1}</h6>
                  <p className="cs_work_subtitle">{feature}</p>
                </div>
              ))}
            </div>
          </div>
